...
class JobList(ResultsObject):
    ...
    def jobs(self):
        jobs = []
        cards = all_or_default(self.soup, 'li.artdeco-list__item')
        for card in cards:
            job = {}
            incomplete = False
            ...
            if location_elem is not None:
                ...
                # print("job location incomplete")
                incomplete = True
            date_elem = one_or_default(card, '.job-card-container__footer-wrapper time', default=None)
            if date_elem is not None:
                job['post_date'] = date_elem['datetime']
            else:
                # print("job post date incomplete")
                incomplete = True
